...
class Display:
    ...
    def generate_events(self, num_events, to_folder, silent=False):
        count = 1
    ...
    def make_graphs(self, img_folder, out_out_vects, gcounts, scale_by_target=False):
        ...
        leading = np.array([event[0] for event in out_out_vects])
        gtype = leading[:,0]
        ...
        ctype = [event[0] for event in self.c_df["particle_id"][:event_count]]
        cidx = np.array([self.partdict.get(id, 0) for id in ctype])
        ccounts = self.c_counts[:event_count]
        cle = [event[0] for event in self.c_df["particle_E"][:event_count]]
        cpx = [event[0] for event in self.c_df["particle_px"][:event_count]]
        cpy = [event[0] for event in self.c_df["particle_py"][:event_count]]
        cpz = [event[0] for event in self.c_df["particle_pz"][:event_count]]
        ...
        for i, a in enumerate(s_graph_list):
            ...
            aa, nbins, _ = ax.hist(a[1], bins=max_v, range=[0,max_v], label='Target', **config)
            ax.hist(a[0], bins=nbins, range=[0,max_v], label='CNF', **config)
...
if __name__ == "__main__":
    mpath = "/global/homes/a/achen899/normalizing-flow/train_out/hadronic_e"
    cpath = "/global/homes/a/achen899/normalizing-flow/gan4hep/nf/config_nf_hadronic.yml"
    dpath = "/global/cfs/cdirs/m3443/data/ForHadronic/train_data/full/data3.hkl"
    ...
    events, counts = hadron_e_display.generate_events(1000, "out_nf.csv")
